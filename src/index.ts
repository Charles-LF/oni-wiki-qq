...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.server.get("/ggwiki/:id", async (ctx) => {
    const pageId = Number(ctx.params.id);
    if (isNaN(pageId)) return (ctx.body = "❌ 无效的页面ID，必须为数字！");

    const [page] = await ctx.database.get("wikipages", { id: pageId });
    if (!page)
      return (ctx.body = `❌ 未找到ID为【${pageId}】的页面，请联系管理员更新缓存！`);

    const targetUrl = `https://oni.wiki/${encodeURIComponent(
      page.title
    )}?variant=zh`;
    ctx.redirect(targetUrl); //重定向至oni.wiki
  });

  // 镜像站路由：klei.vip/bwiki/[id] → 跳转至 wiki.biligame.com/oni/[title]
  ctx.server.get("/bwiki/:id", async (ctx) => {
    const pageId = Number(ctx.params.id);
    if (isNaN(pageId)) return (ctx.body = "❌ 无效的页面ID，必须为数字！");
    ...
    ctx.redirect(targetUrl); //重定向至wiki.biligame.com
  });
  // ==============================================================
